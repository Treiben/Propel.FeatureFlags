...
public interface IMigrationEngine
{
	...
	Task<MigrationResult> SeedAsync(CancellationToken cancellationToken = default);
...
{
	private readonly string _scriptsPath = Path.Combine(Directory.GetCurrentDirectory(), options.ScriptsPath ?? "scripts");
	...
	public async Task<MigrationResult> SeedAsync(CancellationToken cancellationToken = default)
	{
		var result = new MigrationResult();
		try
		{
			...
			{
				logger.LogWarning("Directory {Path} does not exist. Skipping seeding.", _scriptsPath);
				result.Success = false;
				result.Message = $"Directory {_scriptsPath} does not exist.";
				return result;
			}

			var sqlFile = Directory.GetFiles(_scriptsPath, "S*.sql", SearchOption.TopDirectoryOnly)
							.OrderByDescending(f => f)
							.FirstOrDefault();

			if (string.IsNullOrWhiteSpace(sqlFile))
			{
				logger.LogWarning("No seed files matching pattern 'S*.sql' found in {Path} directory. Skipping seeding.", _scriptsPath);
				result.Success = false;
				result.Message = $"No seed files matching pattern 'S*.sql' found in {_scriptsPath} directory. Skipping seeding.";
				return result;
			}
			...
			if (string.IsNullOrWhiteSpace(sqlFile))
			{
				logger.LogWarning("SQL script file {File} is empty. Skipping seeding.", sqlFile);
				result.Success = false;
				result.Message = $"SQL script file {sqlFile} is empty. Skipping seeding.";
				return result;
			}

			var executed = await repository.ExecuteSqlAsync(sql, cancellationToken);
			result.Success = true;
			result.Message = $"Successfully created data in the database.";
		}
		catch (Exception ex)
		{
			logger.LogError(ex, "Failed to create data in the database: {Message}", ex.Message);
			result.Success = false;
			result.Message = ex.Message;
			result.Errors.Add(ex.Message);
		}
