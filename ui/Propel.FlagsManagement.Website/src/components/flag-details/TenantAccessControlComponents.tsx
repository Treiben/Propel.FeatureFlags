...
// Helper function to display blocked tenants with expansion
...
export const TenantAccessSection: React.FC<TenantAccessSectionProps> = ({
	...
	const hasTenantAccessControl = components.hasTenantTargeting ||
		(flag.tenantRolloutPercentage && flag.tenantRolloutPercentage > 0) ||
			...
			{editingTenantAccess ? (
				...
				<div className="text-sm text-gray-600 space-y-1">
					{(() => {
						...
						if (flag.tenantRolloutPercentage > 0 || components.hasTenantTargeting) {
							return (
								<>
									{/* Display rollout percentage */}
									{flag.tenantRolloutPercentage > 0 && (
										<div>Percentage Rollout: {flag.tenantRolloutPercentage || 0}%</div>
									)}
									{components.hasTenantTargeting && (
										<>
											{/* Display allowed tenants using the updated function */}
											{flag.allowedTenants && flag.allowedTenants.length > 0 &&
											...
											{/* Display blocked tenants using the updated function */}
											{flag.blockedTenants && flag.blockedTenants.length > 0 &&
