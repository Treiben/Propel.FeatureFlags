...
namespace FeatureFlags.IntegrationTests.CacheTests.InMemory;
...
public class SetAsync_WithValidFlag(InMemoryTestsFixture fixture) : IClassFixture<InMemoryTestsFixture>
{
	...
	public async Task If_CancellationTokenProvided_ThenCompletesSuccessfully()
	{
		...
		var (flag, _) = new FlagConfigurationBuilder("cancellation-test")
			.WithEvaluationModes(EvaluationMode.On)
			.Build();
		var cacheKey = CacheKeyFactory.CreateCacheKey(flag.Identifier.Key);

		// Act & Assert - Should not throw
		await fixture.Cache.SetAsync(cacheKey, flag, cts.Token);
